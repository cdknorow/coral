...
func TestSessionStateSequences_StoreToPayload_HTTPAndWSParity(t *testing.T) {
	cases := []struct {
		...
		{"needs input survives a later stop", []stateStep{{"notification", permissionNote}, {"stop", "Agent stopped: unknown"}}, wantState{true, false}},
		...
		{"idle notification does not downgrade unresolved needs input", []stateStep{{"notification", permissionNote}, {"stop", ""}, {"notification", idleNote}}, wantState{true, false}},
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			sid := "00000000-0000-0000-0000-0000000005a1"
			...
			if tc.want.needs {
				...
				assert.Equal(t, permissionNote, httpState["waiting_summary"], "summary must be the unresolved request, not later noise")
			} else {
